...
HSV_RANGES = {
    ...
    "drab_orange": ((10,60,100), (25, 140, 200)) ###added for testing purposes
}
...
class YoloClient(Node):
    ...
    def cb(self, msg):
        ...
        for d in dets:
            ...
            roi = f[y1:y2, x1:x2]
            #if roi.size == 0: continue   #changed here

            if self.color_ratio(roi) < 0.05: col=(0, 0, 255)  #changed from 0.15 to 0.05

            col = tuple(map(int, roi.mean(axis=(0,1))))
            label = f"{conf:.2f}"

            cv2.rectangle(f,(x1,y1),(x2,y2),col,2)
            (w,h),_ = cv2.getTextSize(label,0,0.4,1)  ###text with box on side....
